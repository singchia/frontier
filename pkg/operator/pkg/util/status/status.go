...
type Option interface {
	ApplyOption(fc *v1alpha1.FrontierCluster)
...
func Update(ctx context.Context, statusWriter client.StatusWriter, fc *v1alpha1.FrontierCluster, optionBuilder OptionBuilder) (reconcile.Result, error) {
	options := optionBuilder.GetOptions()
	for _, opt := range options {
		opt.ApplyOption(fc)
	}

	if err := statusWriter.Update(ctx, fc); err != nil {
